    ...
    return (<main className="page">
      <div className="container">
        <header className="topbar">
          ...
          <span className="live">
            <span className="live__dot" style={{ background: allOk ? 'var(--ok)' : 'var(--down)' }}/>
            {updatedAt ? `updated ${relativeTime(updatedAt.toISOString())}` : '…'}
          </span>
        </header>

        <section className="banner" style={{ borderColor: allOk ? 'var(--ok-dim)' : 'var(--down-dim)' }}>
          <span className="banner__dot" style={{ background: allOk ? 'var(--ok)' : 'var(--down)' }}/>
          <h1 className="banner__text">{headline}</h1>
          {summary && (<span className="banner__meta">
